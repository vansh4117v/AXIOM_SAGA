...
export default function TicketDetail({ ticketKey, onAnalyse }) {
  ...
  const dto = ticket.ticket || ticket.ticketDto || {};
  const briefing = ticket.briefing || null;
        ...
        <div className="bg-bg-card border border-green-800/30 rounded-xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-sage-muted">
              Confidence: {Math.round((ticket.overall_confidence || 0) * 100)}%
            </span>
          </div>
          {briefing.context_summary && (
            <p className="text-sm text-sage-secondary">{briefing.context_summary}</p>
          )}
          {ticket.execution_plan?.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {ticket.execution_plan.map((a, i) => (
